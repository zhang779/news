...
/**
 * Created by caobin on 2016/12/7.
 */
public class MyViewPagerIndicator extends LinearLayout {
    ...
    //圆角半径
    private float radiusX = 50;
    private float radiusY = 50;
    ...
    private void init() {
       // this.setBackgroundDrawable(getResources().getDrawable(R.drawable.indicator_bg));
        this.setBackgroundResource(R.color.wang_yi_title_bg);
        paint = new Paint();
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (isSetData) {
            ...
            for (int i = 0; i < mDatas.size(); i++) {
                ...
                tv.setTextColor(getResources().getColor(R.color.wang_yi_title_bg));
        ...
        if (getChildCount() > 0) {
            for (int i = 0; i < getChildCount(); i++) {
                if (i == currentPosition) {
                    ((TextView) getChildAt(currentPosition)).setTextColor(getResources().getColor(R.color.wang_yi_title_bg));
                } else {
